...
export default function CIBSVALIDApp() {
  ...
  // Auto-advance: when a domain is completed, scroll to top and move to next domain after 1.5 s
  useEffect(() => {
  ...
  // Auto-scroll to submit block when all domains are done
  useEffect(() => {
    if (screen !== "assessment") return;
    if (allDone) {
      setTimeout(() => {
    ...
    return (
      <div style={rootStyle}>
        ...
        <div style={{maxWidth:600,margin:"0 auto",padding:"16px 12px 40px"}} ref={scrollRef}>
          ...
          {/* Domain completed banner — auto-advances in 1.5 s, no user action needed */}
          {domainJustCompleted && (
